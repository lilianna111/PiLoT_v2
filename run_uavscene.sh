...
read_pose_from_file() {
  local name="$1"
  local pose_file="/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene/poses/${name}.txt"
...
for target_name in "${target_names[@]}"; do
  ...
  if [[ $index -ge 0 ]]; then
    ...
    python /home/amax/Documents/code/PiLoT_v2/main.py \
      --config "/home/amax/Documents/code/PiLoT_v2/configs/feicuiwan_m4t_uavscene.yaml" \
      --init_euler "$euler" \
      --init_trans "$trans" \
      --name "$target_name"
