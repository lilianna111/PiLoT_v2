...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        output_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene/outputs"
    ...
    def rendering_worker(self):
        ...
        while True:
            ...
            if idx < len(self.img_list):
                name = os.path.splitext(os.path.basename(self.img_list[idx]))[0]
            else:
                name = str(idx)
    ...
    def localization_worker(self):
        ...
        # 读取 GT 角度（roll 和 pitch）。
        # 优先按文件名对齐，避免图像排序与 txt 行顺序略有差异时把角度先验喂错帧。
        # 格式：图片名 roll pitch
        # 例如：2_0.png 0.0 40.9
        gt_roll_list = []
        gt_pitch_list = []
                ...
                for line in f:
                    ...
                    if len(parts) >= 3:
                        try:
                            ...
                            angle_name = os.path.basename(parts[0])
                            roll_val = float(parts[1])
                            pitch_val = float(parts[2])
                            gt_roll_list.append(roll_val)
                            gt_pitch_list.append(pitch_val)
                            gt_angle_dict[angle_name] = (roll_val, pitch_val)
                        except ValueError:
        ...
        for idx, (img_path, img_tensor) in enumerate(zip(self.img_list, self.query_list)):
            ...
            # 获取 GT 角度值：优先按文件名匹配，只有在旧数据没有名字时才回退到按序号。
            img_name = os.path.basename(img_path)
            if img_name in gt_angle_dict:
                gt_roll, gt_pitch = gt_angle_dict[img_name]
            elif idx < len(gt_roll_list) and idx < len(gt_pitch_list):
                gt_roll = gt_roll_list[idx]
                gt_pitch = gt_pitch_list[idx]
                if not warned_missing_angle:
                    logging.warning(
                        f"角度 GT 未匹配到文件名，将回退为按序号对齐。当前帧: {img_name}"
                    )
                ...
                if not warned_missing_angle:
                    logging.warning(
                        f"GT 角度既未匹配到文件名，数量({len(gt_roll_list)})也少于图像数量({len(self.img_list)}), "
                        "后续帧将不使用 gt_roll 和 gt_pitch"
                    )
                    warned_missing_angle = True
