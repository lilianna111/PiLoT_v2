...
class BaseRefiner:
    ...
    def _filter_points_by_uncertainty(self,
                                      features_ref: List[torch.Tensor],
                                      rcamera: Camera,
                                      points_3d: torch.Tensor,
                                      p2d_r: torch.Tensor,
                                      visible_r: Optional[torch.Tensor],
                                      top_k: int = 500):
        """用置信度筛点：根据置信度图，挑选 top_k 置信度最高的点。"""
        if (not self.conf.compute_uncertainty or p2d_r is None
                or points_3d is None):
            return points_3d, p2d_r, visible_r, None

        # 拆出置信度通道（最后一通道），选分辨率最高的那一层
        try:
            weights_ref = [feat[-1:] for feat in features_ref]
        except Exception:
            return points_3d, p2d_r, visible_r, None
        if len(weights_ref) == 0:
            return points_3d, p2d_r, visible_r, None

        level = int(np.argmax([w.shape[-1] for w in weights_ref]))
        confidence_map = weights_ref[level].detach()
        if confidence_map.dim() == 3:
            confidence_map = confidence_map.squeeze(0)  # (H, W)

        # 将 2D 像素坐标映射到不确定度图分辨率
        if not torch.is_tensor(p2d_r):
            p2d_r = torch.as_tensor(p2d_r, device=confidence_map.device)
        else:
            p2d_r = p2d_r.to(confidence_map.device)
        if p2d_r.dtype != torch.float32:
            p2d_r = p2d_r.float()

        coords = p2d_r.view(-1, 2).clone()
        scale = float(confidence_map.shape[-1]) / float(rcamera.size[0])
        coords *= scale

        h, w = confidence_map.shape[-2], confidence_map.shape[-1]
        x = coords[:, 0].round().long().clamp_(0, w - 1)
        y = coords[:, 1].round().long().clamp_(0, h - 1)
        confidence_vals = confidence_map[y, x]

        keep_k = min(top_k, confidence_vals.shape[0])
        if keep_k == 0:
            return points_3d, p2d_r, visible_r, confidence_map

        # 置信度越大越好 -> largest=True
        keep_indices = torch.topk(confidence_vals, k=keep_k, largest=True).indices

        if not torch.is_tensor(points_3d):
            points_3d = torch.as_tensor(points_3d, device=confidence_map.device)
        else:
            points_3d = points_3d.to(confidence_map.device)
        points_filtered = points_3d[keep_indices]
        p2d_filtered = p2d_r.view(-1, 2)[keep_indices]

        if visible_r is not None:
            if not torch.is_tensor(visible_r):
                visible_r = torch.as_tensor(visible_r, device=points_filtered.device)
            else:
                visible_r = visible_r.to(points_filtered.device)
            visible_flat = visible_r.view(-1)
            visible_filtered = visible_flat[keep_indices].unsqueeze(0)
        else:
            visible_filtered = visible_r

        logger.debug(f'Confidence filter level {level}: {len(confidence_vals)} -> {keep_k}')
        return points_filtered, p2d_filtered, visible_filtered, confidence_map
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, ref_camera: Camera, render_frame, T_query_initial: Pose, T_render: Pose, Points_3D_ECEF, p2d_r=None,visible_r=None,dd = None, gt_depth=None, gt_roll=None, gt_pitch=None, dsm_path=None, render_T_ecef=None,
        ...
        if 'mul' in refine_conf.keys():
            mul = refine_conf['mul']
        # 1. 确保 p2d_r 是 Tensor (float32)
        if p2d_r is not None and not torch.is_tensor(p2d_r):
            p2d_r = torch.from_numpy(p2d_r).float()
        
        # 2. 确保 visible_r 是 Tensor (bool 或 float)
        if visible_r is not None and not torch.is_tensor(visible_r):
            visible_r = torch.from_numpy(visible_r).bool()
        ...
        if p2d_r is not None and Points_3D_ECEF is not None:
            Points_3D_ECEF, p2d_r, visible_r, _ = self._filter_points_by_uncertainty(
                features_ref_dense, ref_camera_modified, Points_3D_ECEF, p2d_r, visible_r, top_k=500)
            # # 可视化保留下来的点，左侧为 query，右侧为 ref
            # try:
            #     p2d_vis = p2d_r
            #     if torch.is_tensor(p2d_vis):
            #         p2d_vis = p2d_vis.detach().cpu()
            #     p2d_vis = np.asarray(p2d_vis).reshape(-1, 2)
            #     visualize_points_on_images(image_query, render_frame, [], p2d_vis.tolist())
            # except Exception as e:
            #     logger.warning(f'可视化筛选点失败: {e}')
        ...
        ret = self.refine_pose_using_features(features_query, scales_query,
                                            ...
                                            features_ref_dense, scales_ref, p2d_r=p2d_r,visible_r=visible_r, p3d = Points_3D_ECEF, gt_depth=gt_depth, gt_roll=gt_roll, gt_pitch=gt_pitch, dd=dd, mul=mul, origin=self.origin, dsm_path=dsm_path, render_T_ecef=render_T_ecef)
    ...
    def refine_pose_using_features(self,
        ...
        # for idx, level in enumerate(range(len(features_query))):
            ...
            # 1. 获取尺寸
            feat_h, feat_w = F_ref.shape[-2], F_ref.shape[-1] # 128, 128
            cam_w = float(rcamera.size[0]) # 512.0
            
            # 2. 【关键修正】计算统一的缩放因子
            # 无论是否 Padding，必须基于宽度（最长边）计算
            scale = feat_w / cam_w  # 128 / 512 = 0.25
            
            scale_x = scale
            scale_y = scale  # <--- 强制相等！绝对不能用 feat_h / cam_h
            ...
            if p2d_r is not None:
                ...
                p2d_r_scaled = p2d_r.clone()
                p2d_r_scaled[..., 0] *= scale_x
                p2d_r_scaled[..., 1] *= scale_y
                
                # --- 调试打印 (验证 Y 是否变为了 ~43) ---
                # if idx == 0: 
                #     print(f"【修正验证】Level {level}")
                #     print(f"  Scale: {scale:.4f} (应为 0.25)")
                #     print(f"  原始 Y: {p2d_r[0,0,0,1].item()} -> 新 Y: {p2d_r_scaled[0,0,0,1].item()}")
                
                # 4. 维度处理 (防止崩溃)
                # 确保形状是 (1, N, 2)
                if p2d_r_scaled.ndim == 4: 
                    p2d_r_scaled = p2d_r_scaled.squeeze(1) # (1, 1, N, 2) -> (1, N, 2)
                if p2d_r_scaled.ndim == 2:
                    p2d_r_scaled = p2d_r_scaled.unsqueeze(0) # (N, 2) -> (1, N, 2)
            else:
                p2d_r_scaled = None
            # ---------------- 修改结束 ----------------
