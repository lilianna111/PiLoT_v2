...
class DirectAbsoluteCost2:
    ...
    def _center_points_from_wgs84_poses(
        ...
        if K is None:
            ...
            K = [2448, 2048, 1471.0652, 1471.0652, 1172.3576, 1046.3676]
        _, _, fx, fy, cx, cy = K
    ...
    def residual_jacobian_batch_quat(
        ...
        if (
            ...
            if cam_K is None:
                ...
                cam_K = [2448, 2048, 1471.0652, 1471.0652, 1172.3576, 1046.3676]
        ...
        if (
            ...
            # 计算姿态在 ENU 下的旋转矩阵。
            # 这里必须与最终输出链路完全一致：
            # build_c2w_batch 会先把 c2w 的第 2/3 列翻轴，再交给 pixloc_to_osg 提欧拉角。
            # 因此这里也使用同样的 c2w 定义，而不是直接拿原始 R_c2w。
            R_c2w_osg = R_c2w.clone()
            R_c2w_osg[:, :, 1:3] *= -1
            rot_enu_in_ecef_T = rot_enu_in_ecef.transpose(-1, -2)  # [num_init_pose, 3, 3]
            R_pose_in_enu = torch.bmm(rot_enu_in_ecef_T, R_c2w_osg)  # [num_init_pose, 3, 3]
            
            # 直接复用 pixloc_to_osg 同一套欧拉角定义：
            # scipy Rotation.from_matrix(...).as_euler('xyz', degrees=True)
            # 返回顺序为 [pitch, roll, yaw]。
            def _extract_pitch_roll_deg_from_R_pose(R_pose_enu_tensor):
                R_pose_np = R_pose_enu_tensor.detach().cpu().numpy()
                eulers_deg = SciRotation.from_matrix(R_pose_np).as_euler("xyz", degrees=True)
                pitch_deg = torch.from_numpy(eulers_deg[:, 0]).to(device=pose.device, dtype=torch.float64)
                roll_deg = torch.from_numpy(eulers_deg[:, 1]).to(device=pose.device, dtype=torch.float64)
                return pitch_deg, roll_deg

            pitch_base_deg, roll_base_deg = _extract_pitch_roll_deg_from_R_pose(R_pose_in_enu)

            # 角度 Jacobian 使用与最终输出完全相同的提角定义做数值差分，
            # 避免手写 asin/atan2 与 pixloc_to_osg 落在不同欧拉角分支。
            eps_fd = 1e-4
            ...
            for k in range(3):
                ...
                R_c2w_pert_osg = R_c2w_pert.clone()
                R_c2w_pert_osg[:, :, 1:3] *= -1
                R_pose_pert = torch.bmm(rot_enu_in_ecef_T, R_c2w_pert_osg)
                pitch_pert_deg, roll_pert_deg = _extract_pitch_roll_deg_from_R_pose(R_pose_pert)

                # 在度域做 wrap，再转成弧度导数，和下面残差定义保持一致。
                droll_deg = (roll_pert_deg - roll_base_deg + 180.0) % 360.0 - 180.0
                dpitch_deg = (pitch_pert_deg - pitch_base_deg + 180.0) % 360.0 - 180.0
                J_roll_cols.append(torch.deg2rad(droll_deg) / eps_fd)
                J_pitch_cols.append(torch.deg2rad(dpitch_deg) / eps_fd)
            ...
            if self.angle_debug_print:
                sample_n = min(num_init_pose, int(self.angle_debug_max_candidates))
                roll_abs_mean = float(np.mean(np.abs(roll_diff_deg_np)))
                pitch_abs_mean = float(np.mean(np.abs(pitch_diff_deg_np)))
                roll_signed_mean = float(np.mean(roll_diff_deg_np))
                pitch_signed_mean = float(np.mean(pitch_diff_deg_np))
                roll_abs_max = float(np.max(np.abs(roll_diff_deg_np)))
                pitch_abs_max = float(np.max(np.abs(pitch_diff_deg_np)))
                print(
                    "[angle_prior/base] N={} | GT roll={:.3f} pitch={:.3f} | "
                    "res roll mean_abs={:.3f} signed_mean={:.3f} max_abs={:.3f} | "
                    "res pitch mean_abs={:.3f} signed_mean={:.3f} max_abs={:.3f}".format(
                        int(num_init_pose),
                        float(gt_roll_deg_np[0]),
                        float(gt_pitch_deg_np[0]),
                        roll_abs_mean,
                        roll_signed_mean,
                        roll_abs_max,
                        pitch_abs_mean,
                ...
                for i in range(sample_n):
                    print(
                        "[angle_prior/candidate] idx={} pred_roll={:.3f} pred_pitch={:.3f} | "
                        "gt_roll={:.3f} gt_pitch={:.3f} | "
                        "diff_roll={:.3f} diff_pitch={:.3f}".format(
                            i,
                            float(roll_base_deg_np[i]),
                            float(pitch_base_deg_np[i]),
                            float(gt_roll_deg_np[i]),
                            float(gt_pitch_deg_np[i]),
                            float(roll_diff_deg_np[i]),
                            float(pitch_diff_deg_np[i]),
                        )
                    )
                if num_init_pose > sample_n:
                    print(
                        "[angle_prior/candidate] ... {} more candidates omitted".format(
                            int(num_init_pose - sample_n)
                        )
                    )
        ...
        if angle_res is not None:
            ...
            if depth_res is not None:
                ...
                cost_depth_candidates = float(self.depth_selection_weight) * valid_count * cost_depth_robust
                cost = torch.cat([
                    cost_proj_robust,
                    cost_depth_candidates,
                    cost_angle_candidates,
                ], dim=-1)
            else:
                w_loss_depth = None
                cost = torch.cat([
        ...
        if angle_res is not None and w_loss_angle is not None:
            ...
            if self.angle_debug_print:
                gn_proj_np = gn_proj.detach().cpu().numpy().ravel()
                gn_angle_np = gn_angle.detach().cpu().numpy().ravel()
                angle_norm_np = angle_norm_deg.detach().cpu().numpy().ravel()
                gate_np = gate.detach().cpu().numpy().ravel()
                scale_np = angle_scale_per_pose.detach().cpu().numpy().ravel()
                grad_angle_scaled_norm_np = (
                    grad_angle_scaled.norm(dim=-1, keepdim=True).detach().cpu().numpy().ravel()
                )
                grad_total_norm_np = (
                    (grad_proj + grad_angle_scaled).norm(dim=-1, keepdim=True).detach().cpu().numpy().ravel()
                )
                contrib_np = grad_angle_scaled_norm_np / (grad_total_norm_np + 1e-12)
                contrib_np = np.clip(contrib_np, 0.0, 1.0)
                print(
                    "[angle_prior/weight] proj_grad mean={:.4f} | angle_grad mean={:.4f} | "
                    "angle_norm_deg mean={:.3f} max={:.3f} | gate mean={:.3f} min={:.3f} | "
                    "scale mean={:.3f} max={:.3f} | contrib mean={:.3f} max={:.3f} | "
                    "fixed={:.3f} target_ratio={:.3f} selection_weight={:.3f}".format(
                        float(np.mean(gn_proj_np)),
                        float(np.mean(gn_angle_np)),
                        float(np.mean(angle_norm_np)),
                        float(np.max(angle_norm_np)),
                        float(np.mean(gate_np)),
                        float(np.min(gate_np)),
                        float(np.mean(scale_np)),
                        float(np.max(scale_np)),
                        float(np.mean(contrib_np)),
                        float(np.max(contrib_np)),
                        float(self.angle_scale_fixed),
                        float(self.angle_target_ratio),
                        float(self.angle_selection_weight),
                    )
                )
            grad = grad + grad_angle_scaled
            Hess = Hess + Hess_angle_scaled
        elif self.angle_debug_print and (gt_roll is not None or gt_pitch is not None):
            reason = []
            if not self.enable_angle_prior:
                reason.append("enable_angle_prior=False")
            if render_T_ecef is None:
                reason.append("render_T_ecef=None")
            if angle_res is None:
                reason.append("angle_res=None")
            if w_loss_angle is None:
                reason.append("w_loss_angle=None")
