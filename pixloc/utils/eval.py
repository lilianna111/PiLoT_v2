...
def evaluate_xyz(results, gt, only_localized=False):
    ...
    print(out)
    return out
def evaluate_XYZ_EULER(results, gt, only_localized=False):
    """
    Evaluate the accuracy of pose predictions by comparing them with ground truth poses.
...
def evaluate(results, gt, only_localized=False):
    ...
    def _gt_key(pred_name, idx):
        if pred_name in gts:
            return pred_name
        k = pred_name.split('_')[0] + '_0.png'
        if k in gts:
            return k
        base = pred_name.rsplit('.', 1)[0] if '.' in pred_name else pred_name
        k = base + '_0.png'
        if k in gts:
            return k
        if 0 <= idx < len(gt_names_ordered):
            return gt_names_ordered[idx]
        return None
    ...
    for idx, name in enumerate(test_names):
        if name not in predictions:
            ...
            gt_key = _gt_key(name, idx)
            if gt_key is None:
                raise KeyError("eval: no GT for '{}' (tried exact, prefix_0.png, base_0.png, index)".format(name))
            R_gt, t_gt, euler_gt = gts[gt_key]
            R, t, euler = predictions[name]
            # Calculate translation and rotation errors
            # e_trans = -R_gt.T @ t_gt + R.T @ t
            e_t = np.linalg.norm(-t_gt + t, axis=0)
    ...
    out += 'Percentage of test images localized within:'
    # threshs_t = [0.2, 0.5, 1, 2, 5, 10]
    # threshs_R = [0.2, 0.5, 1.0, 1.0, 3.0, 5.0]
    threshs_t = [1, 3, 5, 10]
    threshs_R = [1.0, 3.0, 5.0, 10.0]
...
def evaluate_all(results, gt, only_localized=False):
    """
    Evaluate the accuracy of pose predictions by comparing them with ground truth poses.
    ...
    for name in test_names:
        if name not in predictions:
            ...
            # 计算每个欧拉角的误差
            delta_roll = (euler[0] - euler_gt[0] + 180) % 360 - 180
            e_roll = abs(delta_roll)
            delta_pitch = (euler[1] - euler_gt[1] + 180) % 360 - 180
            e_pitch = abs(delta_pitch)
            delta_yaw = (euler[2] - euler_gt[2] + 180) % 360 - 180
            e_yaw = abs(delta_yaw)
        ...
        errors_R.append(e_R)
        errors_yaw.append(e_yaw)
        errors_tx.append(e_tx)
        errors_ty.append(e_ty)
        errors_tz.append(e_tz)
        errors_roll.append(e_roll)
        errors_pitch.append(e_pitch)
        errors_yaw_individual.append(e_yaw)
    errors_t = np.array(errors_t)
    errors_R = np.array(errors_R)
    errors_yaw = np.array(errors_yaw)
    errors_tx = np.array(errors_tx)
    errors_ty = np.array(errors_ty)
    errors_tz = np.array(errors_tz)
    errors_roll = np.array(errors_roll)
    errors_pitch = np.array(errors_pitch)
    errors_yaw_individual = np.array(errors_yaw_individual)
    # a = errors_R[500:]
    # b = errors_t[500:]
    med_t = np.median(errors_t)
    std_t = np.std(errors_t)
    ...
    # 计算每个分量的中位数和标准差
    med_tx = np.median(errors_tx)
    std_tx = np.std(errors_tx)
    med_ty = np.median(errors_ty)
    std_ty = np.std(errors_ty)
    med_tz = np.median(errors_tz)
    std_tz = np.std(errors_tz)
    med_roll = np.median(errors_roll)
    std_roll = np.std(errors_roll)
    med_pitch = np.median(errors_pitch)
    std_pitch = np.std(errors_pitch)
    med_yaw_ind = np.median(errors_yaw_individual)
    std_yaw_ind = np.std(errors_yaw_individual)
    
    out = '\n整体误差统计:\n'
    out += 'Median errors: {:.3f}m, {:.3f}deg, {:.3f}deg\n'.format(med_t, med_R, med_yaw)
    out += 'Std errors: {:.3f}m, {:.3f}deg, {:.3f}deg\n'.format(std_t, std_R, std_yaw)
    
    out += '\n各分量误差统计:\n'
    out += '平移误差 (X, Y, Z):\n'
    out += '  Median: {:.3f}m, {:.3f}m, {:.3f}m\n'.format(med_tx, med_ty, med_tz)
    out += '  Std: {:.3f}m, {:.3f}m, {:.3f}m\n'.format(std_tx, std_ty, std_tz)
    out += '旋转误差 (Roll, Pitch, Yaw):\n'
    out += '  Median: {:.3f}deg, {:.3f}deg, {:.3f}deg\n'.format(med_roll, med_pitch, med_yaw_ind)
    out += '  Std: {:.3f}deg, {:.3f}deg, {:.3f}deg\n'.format(std_roll, std_pitch, std_yaw_ind)
    ...
    threshs_t = [0.2, 0.5, 1, 2, 5, 10]
    # 角度分量阈值（单位：度）
    threshs_angle = [0.2, 0.5, 1.0, 3.0, 5.0]
    
    out += '\nX平移误差阈值:'
    for th in threshs_t:
        ratio = np.mean(errors_tx < th)
        out += '\n\t{:.0f}cm : {:.2f}%'.format(th * 100, ratio * 100)
    
    out += '\nY平移误差阈值:'
    for th in threshs_t:
        ratio = np.mean(errors_ty < th)
        out += '\n\t{:.0f}cm : {:.2f}%'.format(th * 100, ratio * 100)
    
    out += '\nZ平移误差阈值:'
    for th in threshs_t:
        ratio = np.mean(errors_tz < th)
        out += '\n\t{:.0f}cm : {:.2f}%'.format(th * 100, ratio * 100)
    
    out += '\nRoll角度误差阈值:'
    for th in threshs_angle:
        ratio = np.mean(errors_roll < th)
        out += '\n\t{:.1f}deg : {:.2f}%'.format(th, ratio * 100)
    
    out += '\nPitch角度误差阈值:'
    for th in threshs_angle:
        ratio = np.mean(errors_pitch < th)
        out += '\n\t{:.1f}deg : {:.2f}%'.format(th, ratio * 100)
    
    out += '\nYaw角度误差阈值:'
    for th in threshs_angle:
        ratio = np.mean(errors_yaw_individual < th)
        out += '\n\t{:.1f}deg : {:.2f}%'.format(th, ratio * 100)
